...
if (is_post() && isset($_POST['login_submit'])) {
    ...
    elseif ($user && $user->password === $password) {
        $sql = "UPDATE USER_RESOURCES SET login_attempts = 0, locked_until = NULL WHERE id = ?";
        $stmt = $_db->prepare($sql);
        ...
        if ($remember) {
            $token = sha1(uniqid() . $user->id . rand() . microtime());
            $sql = "UPDATE USER_RESOURCES SET remember_token = ? WHERE id = ?";
            $stmt = $_db->prepare($sql);
    ...
    elseif ($user) {
        ...
        if ($attempts >= 3) {
            ...
            $sql = "UPDATE USER_RESOURCES SET login_attempts = ?, locked_until = ? WHERE id = ?";
            $stmt = $_db->prepare($sql);
            $stmt->execute([$attempts, $locked_until, $user->id]);
            $error = "Too many failed attempts. Account locked for 30 seconds.";
        } else {
            $sql = "UPDATE USER_RESOURCES SET login_attempts = ? WHERE id = ?";
            $stmt = $_db->prepare($sql);
...
if (is_post() && isset($_POST['register_submit'])) {
    ...
    $sql = "SELECT COUNT(*) FROM USER_RESOURCES WHERE email = ?";
    $stmt = $_db->prepare($sql);
    $stmt->execute([$email]);
    $count = $stmt->fetchColumn();

    if ($count == 0) {
        $sql = "INSERT INTO USER_RESOURCES (email, password, name, role) VALUES (?, ?, ?, 'Member')";
        $stmt = $_db->prepare($sql);
